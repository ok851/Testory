...
from desktop_input import sendinput_pointer_at_screen, sendinput_type_text
...
class DesktopAutomation:
    ...
    def _run_visual_pointer(self, step: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            shot = self._capture_failure_screenshot(step)
    ...
    def _perform_visual_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        x, y, score = resolve_visual_click_point(payload)
        self._last_match_score = score
        self._last_coords = (x, y)
        sendinput_pointer_at_screen(x, y, action)
        return {
            "status": "success",
            "action": action,
            "verified": True,
...
def sync_desktop_verify_element(
    ...
    try:
        from desktop_visual_engine import VisualStepPayload, resolve_visual_click_point

        payload = VisualStepPayload.from_json(selector_value)
        x, y, score = resolve_visual_click_point(payload)
        return {
            "success": True,
            "resolved_via": "visual",
            "match_score": score,
            "coords": f"{x},{y}",
            "message": f"视觉匹配成功 score={score:.3f} @ {x},{y}",
        }
