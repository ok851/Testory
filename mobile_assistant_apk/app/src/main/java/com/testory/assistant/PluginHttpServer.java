...
public final class PluginHttpServer extends NanoHTTPD {
    ...
    private JSONObject dispatch(String method, JSONObject params) throws Exception {
        switch (method) {
            ...
            case "stopRecording":
                RecordingSession.stop(appContext);
            ...
            case "pollSteps":
                return pollSteps(params.optInt("limit", 20));
            case "getPageSource":
    ...
    private JSONObject startRecording(JSONObject params) throws Exception {
        boolean screenshotPerStep = params.optBoolean("screenshotPerStep", true);
        AssistantSession.setScreenshotPerStep(screenshotPerStep);
        // PC agent recording: enable mirror path so local drain reads from mirroredSteps
        PluginHttpServer.setAgentRecordingActive(true);
        long caseId = ensureDraftCase();
        if (caseId < 0) {
            return new JSONObject().put("ok", false).put("error", "无法创建录制草稿");
        }
        RecordingSession.start(appContext, caseId, RecordingSession.defaultListener(appContext), true);
        return ok("recording started");
    }

    /** 获取本地草稿用例ID，不存在则创建。 */
    private long ensureDraftCase() {
        try {
            LocalStore store = LocalStore.get(appContext);
            java.util.List<JSONObject> cases = store.listCasesByProject(AssistantSession.LOCAL_PROJECT_ID);
            if (cases.isEmpty()) {
                return store.upsertCase("PC录制草稿", null, AssistantSession.LOCAL_PROJECT_ID, "本地录制");
            }
            return cases.get(0).optLong("id", -1L);
        } catch (Exception e) {
            return -1L;
        }
    }
    ...
    /** Drain steps: read from pendingSteps (local) or mirroredSteps (PC agent), never stall. */
    static org.json.JSONArray drainPendingSteps(int limit) {
        // When PC agent is actively polling, read from mirroredSteps to avoid race
        // For local-only recording, drain directly from pendingSteps
        CopyOnWriteArrayList<JSONObject> source = agentRecordingActive ? mirroredSteps : pendingSteps;
    ...
    private JSONObject pollSteps(int limit) throws Exception {
        ...
        for (int i = 0; i < toDrain; i++) {
            try {
                JSONObject step = pendingSteps.remove(0);
                out.add(step);
