...
from modules.hermes.hermes_config import ensure_hermes_home, hermes_home_dir
from modules.hermes.hermes_gateway_client import HermesGatewayClient, _norm
...
def _inject_hermes_env(env: dict) -> None:
    from modules.hermes.hermes_config import _HERMES_LLM_ENV_KEYS, resolve_hermes_api_server_key
...
def stop_hermes_gateway(*, clear_cdp: bool = True, cleanup_browser: bool = True) -> Dict[str, Any]:
    ...
    try:
        from modules.hermes.hermes_desktop_enable import stop_testory_desktop_mcp
    ...
    try:
        ...
        if already_down:
            ...
            if cleanup_browser:
                try:
                    from modules.ai.ai_external_browser_bridge import force_cleanup_browser
...
def get_bootstrap_status() -> dict:
    ...
    if running:
        try:
            from modules.hermes.hermes_config import hermes_cdp_endpoint_active
    ...
    try:
        from modules.hermes.hermes_config import hermes_desired_llm_fingerprint, hermes_env_llm_snapshot
...
def bootstrap_hermes_services(*, force: bool = False, manual: bool = False) -> dict:
    ...
    with _LIFECYCLE_LOCK:
        ...
        if _BOOTED and not force:
            try:
                from modules.hermes.hermes_config import hermes_desired_llm_fingerprint, resolve_hermes_api_server_key
            ...
            try:
                from modules.hermes.hermes_desktop_enable import ensure_hermes_desktop_control
            ...
            if HermesGatewayClient().health_check(timeout_sec=0.8):
                try:
                    from modules.hermes.hermes_config import hermes_desired_llm_fingerprint
    ...
    try:
        ...
        try:
            from modules.hermes.hermes_config import ensure_hermes_api_server_toolsets
...
def sync_platform_llm_to_hermes() -> dict:
    ...
    from modules.hermes.hermes_config import ensure_hermes_home, sync_platform_llm_credentials_to_hermes_env
