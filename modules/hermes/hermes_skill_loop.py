...
def _learn_selectors_async(plan: Dict[str, Any]) -> None:
    ...
    def _run() -> None:
        try:
            from modules.hermes.ai_hermes_skills import learn_selectors_from_plan
...
def record_selector_healing_event(
    ...
    *,
    ...
    try:
        from modules.hermes.ai_hermes_skills import record_selector_healing
...
def get_alternate_selectors(selector: str) -> List[Dict[str, Any]]:
    ...
    try:
        from modules.hermes.ai_hermes_skills import lookup_alternate_selectors
...
def get_selector_knowledge_stats() -> Dict[str, Any]:
    """对外接口：获取选择器知识库统计。"""
    try:
        from modules.hermes.ai_hermes_skills import get_selector_stats
