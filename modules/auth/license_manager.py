...
def resolve_license_file_path() -> Path:
    ...
    return Path(__file__).resolve().parents[2] / "license.key"
...
class LicenseManager:
    ...
    def _migrate_legacy_license_file(self) -> None:
        ...
        legacy_candidates = (
            ...
            Path(__file__).resolve().parents[2] / "license.key",
        )
    ...
    def features_unlocked_for_open_core(self) -> bool:
        ...
        if (os.environ.get("UAT_OPEN_FEATURES") or "").strip().lower() in (
            ...
            return True
        try:
            from modules.core.deployment_config import is_standalone_mode
