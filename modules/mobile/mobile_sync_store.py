...
def _sync_dirs() -> List[Path]:
    ...
    try:
        from modules.core.install_paths import uat_data_dir  # type: ignore

        candidates.append(Path(uat_data_dir()) / "mobile_sync")
    except Exception:
        pass
    candidates.append(Path(__file__).resolve().parents[2] / "mobile_sync")
    for d in candidates:
        try:
            key = str(d.resolve())
        except Exception:
        ...
        seen.add(key)
        dirs.append(d)
    return dirs or [Path(__file__).resolve().parents[2] / "mobile_sync"]
...
def register_sync_routes(app, *, api_error_handler, login_required, role_required=None):
    ...
    def api_mobile_sync_ai_generate():
        ...
        if mode == "agent":
            try:
                from modules.ai.ai_chat_tool_loop import ChatToolLoopParams, run_unified_agent_blocking
                from modules.ai.ai_local_inference import local_ai_service
    ...
    def api_mobile_sync_captcha_solve():
        ...
        try:
            from modules.ai.ai_vision_local import captcha_vision_solve
