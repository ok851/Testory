...
def _reclick_armed_search_if_needed() -> Dict[str, Any]:
    ...
    try:
        from modules.desktop.desktop_input import force_focus_hwnd, screen_click
...
def _wait_stable_quiet(timeout_ms: Optional[int] = None) -> None:
    ...
    try:
        from modules.ai.screen_tools import wait_screen_stable
...
def refresh_desktop_target_hwnd() -> Dict[str, Any]:
    ...
    if hwnd:
        try:
            from modules.desktop.desktop_input import is_valid_hwnd
...
def capture_desktop_target_foreground(*, step: str = "") -> Dict[str, Any]:
    ...
    try:
        from modules.desktop.desktop_input import get_foreground_hwnd, reclaim_foreground_hwnd, no_focus_steal_enabled
...
def _force_foreground_hwnd(hwnd: int) -> bool:
    ...
    if not hwnd:
        return False
    try:
        from modules.desktop.desktop_input import force_focus_hwnd
...
def _capture_target_hash(hwnd: int = 0) -> str:
    ...
    try:
        from modules.ai.screen_tools import (
            capture_hwnd_png,
...
def observe_screen_texts(*, prefer_foreground: bool = True) -> Dict[str, Any]:
    ...
    try:
        from modules.ai.screen_tools import get_screen_text, clear_ocr_cache
...
def _restore_and_focus_hwnd(hwnd: int) -> bool:
    with _steal_focus_enabled():
        try:
            from modules.desktop.desktop_input import focus_hwnd
...
def _windows_focus_app_impl(app_name: str) -> Dict[str, Any]:
    ...
    if not picked:
        try:
            from modules.desktop.desktop_discovery import list_visible_windows
    ...
    from modules.desktop.desktop_input import reclaim_foreground_hwnd
...
def _resolve_launch_input(app_name: str) -> Tuple[str, str]:
    ...
    if not name:
        return "", ""
    try:
        from modules.desktop.agent_desktop_fastpath import resolve_desktop_launch_target
...
def _launch_process(path: str, is_browser: bool = False) -> None:
    ...
    if is_browser and sys.platform == "win32":
        import subprocess
        try:
            from modules.desktop.desktop_embed_launch import popen_with_embed_hooks
...
def _wait_post_launch_stabilize(is_browser: bool = False):
    ...
    """
    try:
        from modules.ai.screen_tools import capture_for_observation, _is_valid_frame
...
def _windows_launch_app_impl(app_name: str) -> Dict[str, Any]:
    ...
    path = launch_val
    try:
        from modules.desktop.desktop_env_config import smart_resolve_launch_path
...
def _type_text_impl(text: str, *, clear: bool = False, hwnd: int = 0) -> Dict[str, Any]:
    ...
    from modules.desktop.desktop_input import deliver_text_to_hwnd, sendinput_type_text
...
def _ocr_find_candidates(description: str, png: bytes) -> List[Dict[str, Any]]:
    from modules.desktop.desktop_ocr import extract_text_blocks, extract_text_blocks_roi
...
def _verify_typed_text_on_screen(
    ...
    if not token:
        return out
    try:
        from modules.ai.screen_tools import capture_hwnd_png, capture_for_observation, clear_ocr_cache
        from modules.desktop.desktop_ocr import extract_text_blocks_roi, extract_text_from_bytes
...
def _verify_typed_text_vlm(
    ...
    if not token:
        return None
    try:
        from modules.ai.vlm_grounding import is_vlm_ready, get_vlm
        if not is_vlm_ready():
            return None
        from modules.ai.screen_tools import capture_hwnd_png, capture_for_observation
        hwnd = int(hwnd or get_desktop_target().get("hwnd") or 0)
        png = None
        if hwnd:
            png, _ = capture_hwnd_png(hwnd)
...
def _verify_typed_text(
    ...
    *,
    ...
    # 1) UIA 焦点框回读
    try:
        from modules.desktop.desktop_input import uia_get_focused_edit_text
    ...
    if search_context:
        try:
            from modules.desktop.desktop_input import uia_hwnd_tree_contains_text
...
def _try_search_hotkey_shortcut(description: str) -> Optional[Dict[str, Any]]:
    ...
    hwnd = int(get_desktop_target().get("hwnd") or 0)
    try:
        from modules.desktop.desktop_input import deliver_keys_to_hwnd, force_focus_hwnd
        ...
        try:
            from modules.ai.screen_tools import capture_for_observation
...
def _probe_search_accepts_input(hwnd: int) -> Dict[str, Any]:
    ...
    """
    from modules.desktop.desktop_input import get_foreground_hwnd
...
def _activate_wechat_search_for_input() -> Dict[str, Any]:
    ...
    from modules.desktop.desktop_input import deliver_keys_to_hwnd, get_foreground_hwnd, screen_click
...
def windows_wait(
    ...
    if cond in ("desktop_change", "window_change", "桌面变化", "窗口变化"):
        from modules.desktop.desktop_input import wait_for_desktop_change
    ...
    if cond.startswith("window:") or cond.startswith("title:"):
        from modules.desktop.desktop_input import wait_for_window_title_keyword
...
def dispatch_windows_or_screen_tool(name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if n == "get_screen_description":
        from modules.ai.screen_tools import get_screen_description
