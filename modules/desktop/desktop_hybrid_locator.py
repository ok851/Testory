...
def _effect_keyword_from_step(step: Dict[str, Any]) -> str:
    from modules.desktop.desktop_input import infer_effect_keyword
...
def _try_uia_core_locate(step: Dict[str, Any], off_x: int, off_y: int, tw: int, th: int) -> Optional[DesktopResolveResult]:
    ...
    try:
        from modules.desktop.desktop_uia_core import find_element_by_acc_name
...
def _try_ocr_locate(
    ...
    try:
        ...
        if not (ax or ay):
            ...
            try:
                from modules.desktop.desktop_input import get_cursor_pos
...
def _try_dpi_calibration(x: int, y: int) -> Tuple[int, int]:
    """DPI坐标校准。"""
    try:
        from modules.desktop.desktop_precise_locator import get_dpi_scale_factor
...
def resolve_desktop_click_point(step: Dict[str, Any]) -> DesktopResolveResult:
    ...
    try:
        from modules.desktop.desktop_shell_application import try_resolve_shell_application_step
    ...
    try:
        from modules.desktop.desktop_shell_listview import try_resolve_shell_listview_step
    ...
    if snap:
        ...
        # 嵌入式 Chromium DOM 回放
        try:
            from modules.desktop.desktop_embed_cdp import resolve_embed_click_point
    ...
    if snap:
        ...
        if sel.get("resolved_via") == "win32":
            try:
                from modules.desktop.desktop_uia_snapshot import resolve_win32_click_point
