...
def _capture_impl(x: int, y: int) -> SnapshotCaptureResult:
    ...
    try:
        from modules.desktop.desktop_shell_listview import capture_desktop_icon_snapshot_at_point
    ...
    try:
        from modules.desktop.desktop_uia_core import capture_element_at_point
    ...
    try:
        from modules.desktop.desktop_dialog_handler import capture_dialog_element_at_point
    ...
    try:
        from modules.desktop.desktop_win32_snapshot import capture_win32_element_at_point
...
def _peek_impl(x: int, y: int, *, wake_app: bool = True) -> ElementPeekResult:
    ...
    try:
        ...
        if core_result.get("ok"):
            ...
            # 唤醒后再试一次
            try:
                from modules.desktop.desktop_uia_core import (
