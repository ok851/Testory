...
from modules.ai.ai_multi_provider import dispatch_chat_completion_messages, dispatch_chat_stream
from modules.core.logger import uat_logger
from modules.web.embedded_browser_client import embedded_gateway_enabled
from modules.ai.agent_gateway_client import agent_tool_result_max_chars, get_agent_gateway_client
...
def _generate_retry_description(original: str, ocr_texts: List[Dict[str, Any]]) -> str:
    ...
    if not ocr_texts:
        return original
    from modules.web.element_confidence import ElementConfidence
    best_score = 0.0
    best_text = ""
...
def _auto_open_wechat_search_hit_after_type(
    *,
    ...
    if enter_ok:
        ...
        try:
            from modules.desktop.windows_desktop_tools import mark_compose_input_phase
...
def _maybe_persist_desktop_run_memory(
    ...
    *,
    ...
    if not winish:
        return
    try:
        from modules.desktop.desktop_run_memory import apps_from_meta, record_successful_run
...
def _emit_tool_registry_audit_event() -> None:
    try:
        from modules.execution.execution_events import ExecutionEventCollector, TOOL_REGISTERED
        from modules.ai.agent_tool_registry import describe_registry
...
def _dispatch_desktop_or_screen_tool(name: str, args: Dict[str, Any]) -> str:
    ...
    from modules.desktop.windows_desktop_tools import WINDOWS_COMPAT_TOOL_NAMES
...
def _is_cross_end_agent_tool(name: str) -> bool:
    try:
        from modules.mobile.mobile_cross_end_tools import DESKTOP_ALIAS_TOOL_NAMES, MOBILE_TOOL_NAMES
...
def _dispatch_cross_end_agent_tool(
    ...
    *,
    ...
    from modules.mobile.mobile_cross_end_tools import dispatch_cross_end_tool
...
def _get_bridge_page_state() -> Dict[str, str]:
    ...
    try:
        from modules.ai.ai_external_browser_bridge import get_page
        page = get_page()
...
def _prepare_unified_agent_meta(params: "ChatToolLoopParams", plat: str) -> Dict[str, Any]:
    ...
    if uid > 0:
        try:
            from modules.ai.agent_unified_session import get_or_create_session, merge_cross_end_vars
...
def _persist_unified_agent_session(params: "ChatToolLoopParams", meta: Dict[str, Any], reply: str = "") -> None:
    ...
    if uid <= 0:
        return
    try:
        from modules.ai.agent_unified_session import merge_cross_end_vars, set_session_meta
...
def run_ai_chat_with_tools(
    *,
    ...
    try:
        from modules.ai.agent_intent import _score_surfaces, _first_url
    ...
    from modules.desktop.windows_desktop_tools import SCREEN_TOOL_NAMES, WINDOWS_TOOL_NAMES
        ...
        while idx_tc < len(pending_calls):
            ...
            if name in ("hermes_execute", "openclaw_execute"):
                ...
                if name in SCREEN_TOOL_NAMES:
                    ...
                    try:
                        from modules.ai.ai_screen_observer import (
                    ...
                    if obs_count == 0:
                        try:
                            from modules.ai.ai_screen_observer import ensure_screen_observation_cached
                            meta["last_screen_obs"] = ensure_screen_observation_cached(meta)
                            meta["obs_count"] = 1
                        except Exception:
                            pass
...
def run_ai_chat_with_tools_stream(
    *,
    ...
    # 轻量桌面记忆 hint（若有）
    try:
        from modules.desktop.desktop_run_memory import hint_for_app
    ...
    for round_idx in range(_max_tool_rounds()):
        ...
        while idx_tc < len(pending_calls):
            ...
            if name in ("hermes_execute", "openclaw_execute"):
                ...
                if tool_evs:
                    from modules.ai.ai_action_recorder import ActionRecorder as _AR
