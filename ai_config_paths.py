...
def resolve_install_root() -> Path:
    try:
        from install_paths import resolve_install_root as _rir
...
def _search_data_file(filename: str) -> Path | None:
    ...
    _add(Path(__file__).resolve().parent / name)

    try:
        from install_paths import resource_root
...
def ai_model_registry_path() -> Path:
    ...
    try:
        from desktop_user_data import resolve_user_data_dir
